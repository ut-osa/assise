...
typedef enum {SEQ_WRITE, SEQ_READ, SEQ_WRITE_READ, RAND_WRITE, RAND_READ, SIGNAL, NONE} test_t;
...
int do_wait = 0;	// by default, block until start signal received
...
#if 1
		int dev_id = 0;
		char *device_id = device_id = getenv("DEV_ID");
...
ssize_t mem_bench::io_write(int fd, void *buf, size_t count)
{
	...
	switch (test_mode) {
		case SYSCALL: 
			...
			break;
		case MEMCPY:
		        //printf("[id %d] write - addr %lu size %lu\n", id, start_addr + offset, count);
...
{
	std::cerr << "usage: " << prog  
		<< " <wr/sr/sw/rr/rw/s> <fs/nvm/dram/ssd>" 
		<< " <size: X{G,M,K,P}, eg: 100M> <IO size, e.g.: 4K> <# of threads>"
		<< " [-nont|-numa|-zero|-ioat|-wait]"
		<< endl;
}
...
void print_aggregate_stats(double *worker_tput, int n_workers, char *msg)
{
	double min_bps = DBL_MAX;
	for(int i=0; i < n_workers; i++) {
		min_bps = min(min_bps, worker_tput[i]);
	}

	printf("%s Aggregate throughput: %3.3f GB/s\n",msg, (float)(min_bps * n_threads / (float)(1024.0 * 1024.0 * 1024.0)));
}
#endif
...
restart:
   for (int i = 0; i < argc; i++) {
     ...
     else if (strncmp("-wait", argv[i], 5) == 0) {
	 do_wait = 1;
         dash_d = i;
	 argc = adjust_args(dash_d, argv, argc, 1);
	 goto restart;
...
int main(int argc, char *argv[])
{
	...
	if(test_type == SIGNAL) {
		cout << "sending start signal to dmabench processes" << endl;
		shm_i = (int*)utils::create_shm(fd, res);
		*shm_i = 0;
		return 0;
	}
	...
	if(do_wait) {
		shm_i = (int*)utils::create_shm(fd, res);
		*shm_i += 1;
		cout << "waiting for start signal (pid: " << getpid() << " shm_i:"<< *shm_i <<"). Run ./dmabench s in another terminal." << endl;
